...
* Some functions are knowingly unsupported and may never be implemented:
  ...
  1. Shape dimming after playing an effect.
  1. Effects triggered by mouse clicks on a specific shape.
  1. The shaking and blinking emphasis effects, due to a PowerPoint bug.
  1. Processing of effects applied to individual shapes of SmartArt objects or
  ...
  1. Duration for emphasis effects (e.g., until next click or until the end of
  the slide): all emphasis effects last until the end of the slide.
  1. Accurate rendering of color effects: they do not perfectly match
  PowerPoint's behavior (which is not obvious to reverse engineer) but still
  provide an acceptable emphasis effect.
  1. Most emphasis and motion effects that apply to a single text paragraph
  ...
  supported.
  1. Text effects applied to paragraphs containing a mixture of standard text and
  equations in the new native PowerPoint 2013 format (i.e., not Equation Editor
  objects).
  1. Accurate alignment of shapes in a group. This is hardly obtainable, as it
  ...
  1. Rasterized shape scaling. Indeed, PowerPoint applies effects to rasterized
  ...
  1. Accurate processing of emphasis effects affecting fill/line colors. For
  ...
  1. Handling the auto-fit property for text frames, due to some limits in the
  ...
  1. Accurate rendering of some rotation effects. During the presentation
  ...
  has "wandered around".
  1. Exit/entry effects applied to shapes that are part of a slide layout are only
  ...
  1. Adjustment of slide numbers on a PPTX file that is imported into PowerPoint
  <=2003 using the Microsoft Office Compatibility Pack.
  1. Adjustment of (dynamic) slide numbers that appear in standard text boxes
  ...
  1. Handling of bullet symbols in itemized lists: bullets may sometimes be lost
  ...
  1. Animations in slide masters.
  1. Something else I am not aware of.
